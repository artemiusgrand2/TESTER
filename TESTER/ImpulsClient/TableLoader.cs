...
{
	/// <summary>
	/// Класс для загрузки импульса из входного файла
	/// </summary>
	public class ImpulseRecord
	{
		public string Name;
		public int Box;
		public int Contact;
	...
	public static class TableLoader
	{
		...
		public static void ReadImpulseTable(string fileName, bool is_tu, out ImpulseRecord[] records)
		{
			if(fileName == null)
...
// 			}
			try
			{
				...
				records = new ImpulseRecord[count];
				count = 0;
				while ((input = sr.ReadLine()) != null)
					if (Regex.IsMatch(input, pattern))
					{
						int nameStartPos = input.IndexOf('\'', 0) + 1;
						records[count] = new ImpulseRecord();
						records[count].Name = input.Substring(nameStartPos, input.IndexOf('\'', nameStartPos) - nameStartPos);
						count++;
					}
		...
		/// <exception cref="FileNotFoundException">Если fileName содержит неправильный путь к файлу или такого файла нет.</exception>
		/// <exception cref="IOException">Если произошла ошибка при чтении файла.</exception>
		public static void ReadImpulsesIO(string fileName, /*ImpulsesTableType tableType,*/ int matrix, ImpulseRecord[] records)
		{
			if(records == null)
...
// 			}
			...
			{
				string input;
				ImpulseRecord tmp = new ImpulseRecord();
				while((input = sr.ReadLine()) != null)
				{
...
// 					}
        ...
        public static void GetStdImpulses(string path, int station, bool is_tu, out ImpulseRecord[] records)
        {
            records = null;
            string fileName = "";
            if (path == null)
            ...
            if (fileName == "")
                throw new FileNotFoundException(string.Format("File {0} not found in directory {1}", pattern, info.FullName));
